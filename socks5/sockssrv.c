...
static int connect_socks_target(unsigned char *buf, unsigned int  n, struct client *client) {
...
static int is_authed(union sockaddr_union *client, union sockaddr_union *authedip) {
	if(authedip->v4.sin_family == client->v4.sin_family) {
		...
		unsigned int  cmpbytes = af == AF_INET ? 4 : 16;
...
static enum authmethod check_auth_method(unsigned char *buf, unsigned int  n, struct client*client) {
...
static void copyloop(int fd1, int fd2) {
	...
	while(1) {
		...
		char buf[1024];
		unsigned int  sent = 0, n = recv(infd, buf, sizeof buf,0);
		if(n <= 0) return;
		while(sent < n) {
			unsigned int  m = send(outfd, buf+sent, n-sent,0);
			if(m < 0) return;
			sent += m;
		}
	}
...
static enum errorcode check_credentials(unsigned char* buf, unsigned int  n) {
	if(n < 5) return EC_GENERAL_FAILURE;
...
static void collect(sblist *threads) {
	...
	for(i=0;i<sblist_getsize(threads);) {
		...
		if(thread->done) {
			WaitForSingleObject(thread->pt, INFINITE);
...
static int usage(void) {
	fprintf(stderr,
		"MicroSocks SOCKS5 Server (Windows port by @unamer)\n"
		"------------------------\n"
...
static void zero_arg(char *s) {
	unsigned int  i, l = strlen(s);
	for(i=0;i<l;i++) s[i] = 0;
}

int main(int argc, char** argv) {
	auth_ips_mutex = CreateMutex(0, 0, 0);
	...
	while ((argc > 1) && (argv[1][0] == '-'))
	{
		...
		case 'u':
			++argv;
			--argc;
			auth_user = _strdup(argv[1]);
			break;

		case 'P':
			++argv;
			--argc;
			auth_pass = _strdup(argv[1]);
			break;
	...
	server = &s;
	unsigned int  stacksz = MAX(8192, PTHREAD_STACK_MIN);  /* 4KB for us, 4KB for libc */
	...
	while(1) {
		...
		curr->pt = CreateThread(0, stacksz, (LPTHREAD_START_ROUTINE)clientthread, curr, 0, 0);
		if (!curr->pt)
		{
			dolog("Error CreateThread");
		}
	}
}
